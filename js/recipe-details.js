...
const name = namewithoutspace.replace(/%(20)/g, " ")
let finalname = name.replace(/%27/g, "'")
...
form.addEventListener('submit', (e) => {
    ...
    commentNumber.textContent = recipe.iscomment.length + 1 + (commentsDatabase?.[recipe.name]?.length ?? 0);
});
const commentsDatabase = JSON.parse(localStorage.getItem('comments'));
if (commentsDatabase) {
    ...
    commentNumber.innerText = recipe.iscomment.length + (commentsDatabase?.[recipe.name]?.length ?? 0);
}
